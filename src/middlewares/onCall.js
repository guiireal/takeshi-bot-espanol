...
const ANTI_CALL_MESSAGE = `${BOT_EMOJI} 📵 ¡Las llamadas están prohibidas en este grupo!`;
...
export async function onCall({ socket, calls }) {
  ...
  for (const call of calls) {
    try {
      ...
      errorLog(
        `Error al aplicar anti-call. ¡Verifique si soy administrador del grupo! Detalles: ${error.message}`,
      );
    }
  }
}
