...
function ensureGroupIncident(socket, remoteJid) {
  ...
  incident.closePromise = (async () => {
    await runStep(
      ...
      "Error al cerrar el grupo por anti-payment.",
    );
...
function scheduleGroupReopen(socket, remoteJid) {
  ...
  incident.reopenTimer = setTimeout(() => {
    runStep(
      ...
      "Error al abrir el grupo por anti-payment.",
    ).finally(() => {
...
async function runDefense({ socket, remoteJid, userLid, messageKey }) {
  ...
  await Promise.all([
    ...
    socket
      ...
      .catch((error) => {
        errorLog(
          `Error al eliminar al miembro por anti-payment. Detalles: ${error.message}`,
        );
      }),
    messageKey
      ? runStep(
          ...
          "Error al eliminar el mensaje de pago.",
        )
      : Promise.resolve(),
  ]);
