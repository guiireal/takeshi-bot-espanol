...
export async function deletePaymentMessage({
  ...
  if (!id || !participant) {
    throw new Error(
      "¡No fue posible identificar el mensaje de pago para eliminarlo!",
    );
  }
  ...
  try {
    ...
    await socket.sendMessage(
      remoteJid,
      {
        text: `${BOT_EMOJI} ¡Mensaje de pago eliminado!`,
        edit: {
          remoteJid,
          fromMe: true,
          id: dummyMessageId,
    ...
    throw new Error(
      "¡No fue posible eliminar el mensaje de pago! Inténtelo nuevamente.",
    );
  }
}
