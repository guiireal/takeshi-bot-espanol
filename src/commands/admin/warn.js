  ...
  handle: async ({
    ...
    try {
      ...
      if (targetLid === userLid) {
        throw new DangerError("No puedes se advertir!");
      }

      if (targetLid === BOT_LID || targetLid === OWNER_LID) {
        throw new DangerError("No es posible advertir este usuario.");
      }

      const reason = args.slice(1).join(" ") || "Advertencia genérica";
      ...
      await sendReply(
        `⚠️ *@${targetLid.split("@")[0]}* foi advertido!\n` +
          `Motivo: _"${reason}"_\n` +
          `Total: ${newCount}/${limit} advertencias`,
        [targetLid],
      );
      ...
      if (newCount >= limit) {
        ...
        await sendReply(
          "❌ Limite de advertencias atingido. Usuario removido.",
        );
      }
    } catch (error) {
