...
import { processAnimatedSticker } from "../../services/sticker.js";
import { getRandomName } from "../../utils/index.js";

export default {
  name: "bratvid",
  description: "Genera sticker animado no estilo brat com o texto informado.",
  ...
  handle: async ({
    ...
    if (!fullArgs.length) {
      throw new InvalidParameterError(
        "Debes indicar o texto que deseja transformar em sticker animado.",
      );
    }
    ...
    if (!response.ok) {
      ...
      await sendErrorReply(
        `Ocurrió un error ao executar uma chamada remota para a Spider X API no comando bratvid!\n      \n📄 *Detalhes*: ${data.message}`,
      );
      return;
    }
    ...
    try {
      ...
      finalStickerPath = await processAnimatedSticker(inputPath, metadata);
