...
export default {
  name: "brat",
  description: "Genera imagen no estilo brat com o texto informado.",
  commands: ["brat"],
  ...
  handle: async ({
    ...
    if (!fullArgs.length) {
      throw new InvalidParameterError(
        "Debes indicar o texto que deseja transformar em imagen.",
      );
    }
    ...
    if (!response.ok) {
      ...
      await sendErrorReply(
        `Ocurrió un error ao executar uma chamada remota para a Spider X API no comando brat!\n      \n📄 *Detalhes*: ${data.message}`,
      );
      return;
    }
