...
keyv = __toESM(keyv, 1);
//#region src/store.ts
var NodeCacheStore = class extends hookified.Hookified {
	_keyv;
	_ttl;
	_stats = new _cacheable_utils.Stats({ enabled: true });
	_useClones;
	_deleteOnExpire;
	...
	constructor(options) {
		...
		this._keyv = options?.store ?? new keyv.default();
	...
	*/
	async flushAll() {
		...
		this._values.clear();
		this._ttls.clear();
		this._stats = new _cacheable_utils.Stats({ enabled: true });
		this.emit("flush");
	}
	/**
	...
	*/
	resolveTtl(ttl) {
		...
		if (typeof effectiveTtl === "string") return (0, _cacheable_utils.shorthandToMilliseconds)(effectiveTtl);
...
var NodeCache = class extends hookified.Hookified {
	options = {
		stdTTL: 0,
		checkperiod: 600,
		useClones: true,
	...
	*/
	...
	getExpirationTimestamp(ttlInSeconds) {
		if (typeof ttlInSeconds === "string") return (0, _cacheable_utils.shorthandToTime)(ttlInSeconds);
...
exports.NodeCacheErrors = NodeCacheErrors;
exports.NodeCacheStore = NodeCacheStore;
